...
def parse_phenix(
    ...
    cmd = (
        """grep -E '<URL>|<PositionX Unit="m">|<PositionY Unit="m">|<AbsTime>' '"""
        + index_file
        + """' > '"""
        + outfile
        + """'"""
    ...
    rows = [
        str(x).zfill(2) for x in rows
    ]  # need to add this since otherwise we wont be able to correctly match to wells with more than 1 digit id
    ...
    for old, new in tqdm(
        zip(df.Image_files.tolist(), df.new_file_name.tolist()),
    ...
    print(
        "Parsing Phenix data completed, total time was ", str(endtime / 60), "minutes."
    )
    ...
    if WGAbackground != False:
        ...
        for file_name in os.listdir(outdir):
            ...
            if WGAbackground in file_name:
                ...
                destination = os.path.join(
                    outdir, file_name.replace(WGAbackground, "WGAbackground")
                )
                # copy only files
        ...
        print(
            "Parsing Alexa488 data completed, total time was ",
            str(endtime / 60),
            "minutes.",
        )


def parse_phenix_40X_slide(
    phenix_dir,
    flatfield_exported=True,
    downsampled=False,
    WGAbackground=False,
    ...
    for well in wells:
        for i, row in enumerate(rows):
            if i == 0:
                ...
                df.loc[(df.Well == well) & (df.Row == row), "Y_pos"] = (
                    df.loc[(df.Well == well) & (df.Row == row), "Y_pos"]
                    + int(max_y)
                    + int(1)
                )
    ...
    if compress_cols:
        for i, well in enumerate(wells):
            if i == 0:
                ...
                df.loc[(df.Well == well), "X_pos"] = (
                    df.loc[(df.Well == well), "X_pos"] + int(max_x) + int(1)
                )
